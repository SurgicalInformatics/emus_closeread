...
library(pins)
board = board_connect()
emus_dem = pin_read(board, "rots/emus_dem_op_30day")
emus_colours = c("#d73027", "#fdae61", "#66bd63")
```

```{r}
weekly_recruitment = emus_dem %>% 
  mutate(week = floor_date(form_started, unit = "weeks", week_start = 1)) %>% 
  count(week) %>% 
...
pred_thisyear  = augment(lm_thisyear,  filter_sets(study_dates, "thisyear"))  %>% mutate(pred_length = "From Feb'25")
...
plot1 = weekly_recruitment %>% 
  ...
  scale_y_continuous(limits = c(0, 2000), expand = c(0.1, 0.1)) +
...
plot2 = weekly_recruitment %>% 
  ...
  labs(y = "Number of patients",
       x = "") +
  annotate("text", x = ymd("2025-06-01"), y = 1000, 
...
plot3 = weekly_recruitment %>% 
  ...
  labs(y = "Number of patients",
       x = "") +
  annotate("text", x = ymd("2025-06-01"), y = 1700, 
           label = "Number of patients based on \n Feb-Jun 2025 recruitment rate") +
  geom_curve(
    data = arrows, aes(x = x1, y = y1, xend = x2, yend = y2),
    arrow = arrow(length = unit(0.08, "inch")), linewidth = 0.5,
    color = "gray20", curvature = 0.3)
